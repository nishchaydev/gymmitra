...
-- Only allow gyms to see their own assigned registration code, not all unassigned codes.
-- To validate/claim a code without broad SELECT, use a controlled server-side RPC.
CREATE POLICY "Gyms can view their registration codes" ON "RegistrationCode" FOR SELECT USING (id IN (SELECT "registrationCodeId" FROM "GymProfile" WHERE "userId" = (select auth.uid())::text));
