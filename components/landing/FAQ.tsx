    ...
    return (
        <section id="faq" className="py-24 bg-slate-50">
            <div className="container px-4 md:px-6 mx-auto max-w-3xl">
                ...
                <Accordion type="single" collapsible className="w-full bg-white rounded-2xl border border-slate-200 p-2 shadow-sm">
                    {faqs.map((faq, i) => (
                        <AccordionItem key={i} value={`item-${i}`} className="border-b-0 px-4">
                            <AccordionTrigger className="text-left text-lg font-semibold text-slate-800 hover:text-brand-secondary py-6 [&[data-state=open]]:text-brand-secondary">
                                {faq.q}
                            </AccordionTrigger>
                ...
                {/* Final CTA */}
                <div className="mt-20 text-center bg-brand-secondary rounded-3xl p-12 relative overflow-hidden">
                    {/* Background Patterns */}
                    ...
                    <p className="text-slate-600 text-lg mb-8 max-w-xl mx-auto relative z-10">
                        Join 50+ other smart gym owners today. No credit card required.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center relative z-10">
                        <Link href="/login?view=register">
                            <Button size="lg" className="h-14 px-8 text-lg rounded-full bg-white text-slate-900 hover:bg-slate-50 font-bold w-full sm:w-auto">
